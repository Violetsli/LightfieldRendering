...
class ConvTemplate(nn.Module):
    ...
    def forward(self, encoding):
        ...
        #print(self.template1(encoding))
        #encoding.view(-1, 1024, 1, 1, 1)
        #encoding1 = self.template1(encoding)
...
class AffineMixWarp(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.warps = nn.Sequential(
                nn.Linear(1024, 128), nn.LeakyReLU(0.2),
                nn.Linear(128, 3*16))
        ...
        self.weightbranch = nn.Sequential(
                nn.Linear(1024, 64), nn.LeakyReLU(0.2),
    ...
    def forward(self, encoding):
        ...
        print("weight.size(1),grid.shape,warpedweight",weight.size(1), self.grid.shape,warpedweight.shape, warpt.shape, warprot.shape, warps.shape)

        return warp.permute(0, 4, 1, 2, 3)

def getwarp(warptype, **kwargs):
    print("warptype,",warptype)
    if warptype == "conv":
...
class Decoder(nn.Module):
    ...
    def forward(self, encoding, viewpos, losslist=[]):
        ...
        print("self.viewconditioned:", self.viewconditioned)
        ...
        encoding = encoding.reshape(1,1024)
        print("encoding:",encoding.shape)

        warp = self.warp(encoding) if self.warp is not None else None


        print("self.globalwarp", self.globalwarp)
        if self.globalwarp:
