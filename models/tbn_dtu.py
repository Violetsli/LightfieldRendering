...
class TBN(nn.Module):
    #def __init__(self, num_in_layers, num_out_layers, args, vol_dim=40, num_features=800,tensor_type='torch.FloatTensor', ):
    def __init__(self, num_in_layers, num_out_layers, vol_dim=64, num_features=2048,tensor_type='torch.FloatTensor', ):
        ...
        #
        self.deconv3d_32_1 = Conv(Cube_channel,Cube_channel, 3, 1, is_3d_conv=True)
        self.deconv3d_32_2 = Conv(Cube_channel, Cube_channel, 3, 1, is_3d_conv=True)
        self.deconv3d_32_3 = Conv(Cube_channel, Cube_channel, 3, 1, is_3d_conv=True)
        self.deconv3d_32_4 = Conv(Cube_channel, Cube_channel, 3, 1, is_3d_conv=True)
        ...
        self.deconv3d_1 = Conv(Cube_channel, Cube_channel, 3, 1, is_3d_conv=True)
        self.deconv3d_2 = Conv(Cube_channel, Cube_channel, 3, 1, is_3d_conv=True)

    def forward(self, num_inputs_to_use, data):
        ###test
        return NULL
        
    def encode(self, src_rgb):
        ...
        supp = torch.zeros([x.shape[0], x.shape[1],4,400])
        x = torch.cat((x, supp.to(x.device)), 2)
        #print("new x input shape:", x.shape)
        ...
        x_out = self.upconv3_2d_encode(x3)


        #print("x_out,skip3:", x3.shape, x_out.shape,skip2.shape)
        ...
        #print("x_out.shape:", x_out.shape, skip1.shape)
        ...
        #else:
        x_out = x_out[:,:,:75,:]
        #print("final x_out.shape:", x_out.shape)
